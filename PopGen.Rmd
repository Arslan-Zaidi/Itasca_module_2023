...
The goal of this guide is to illustrate the Hardy-Weinberg principle through simulations in R. The guide assumes that you have a basic knowledge of genetics and have completed Part 1 of the module.
...
As one of the most fundamental concepts in genetics, Hardy-Weinberg principle, also known as Hardy-Weinberg equilibrium (HWE), provides a simple null hypothesis asserting that in the absence of other evolutionary forces, allele and genotype frequencies in a population remain unchanged across generations.

The HWE rests on a number of assumptions leading to an equilibrium state in a diploid population, including
...
**Q.** How realistic are these assumptions?
...
We begin by rewriting the simulation code as a function so that we can run the same code multiple times without repeatedly typing the same code. A function is declared by using the key word 'function' and could include 'argument(s)' for versatility. In this simulation, the arguments are the number of individuals sampled and the frequency of the A allele.
...
Now, let's compute the average proportions of individual genotypes
