---
title: "Introduction to Hardy-Weinberg equilibrium"
...
The purpose of this guide is to illustrate the Hardy-Wenberg principle through simulations in R. The guide assumes that you have a basic knowledge of genetics.
...
# the sample function draws from the different choices given
...
Let's do this more systematiclaly by replicating the result multiple times and averaging over replicates. Let's also try a different starting allele frequency.
...
Let's try calcualating the genotype frequencies for different allele frequencies and plotint the result.

```{r}

#setup a vector with allele frequencies ranging from 0 to 1
f = seq(0, 1, 0.1)
mat1 = matrix(NA, nrow = length(f), ncol = 3)
for(j in 1:length(f)){
  mat = matrix(NA, nrow = 1000, ncol = 3)
  for(i in 1:1000){
    #use the function to run the block of code
    mat[i,] = create_genotype(fa = f[j], ninds = 1000)
  }
  
  #calculate the mean genotype across replicates
  mat1[j,] = apply(mat, 2, mean)
}

mat1 = data.frame(mat1)
colnames(mat1) = c("GG","GA","AA")
mat1$freq = f
...
That you can predict the genotype frequencies in a population from the allele frequencies is part of the Hardy-Weinberg equilibrium. This principle states that but if:
...
As you can see, the Hardy-Weinberg principle predicts the 
